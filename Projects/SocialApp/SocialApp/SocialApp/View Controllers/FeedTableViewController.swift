...
class FeedTableViewController: UITableViewController {
    ...
    func retrieveTweets() {
        ...
        if let account = selectedAccount {
            ...
            if let request = SLRequest(forServiceType: SLServiceTypeTwitter,requestMethod: SLRequestMethod.GET, url: requestURL, parameters: [:]) {
                request.account = account
                request.perform()
                    {
                        responseData, urlResponse, error in if(urlResponse?.statusCode == 200) {
                            if(urlResponse?.statusCode == 200) { do {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cachedImage = image {
            cell.tweetUserAvatar.image = cachedImage } else {
            ...
            operationQueue.addOperation() {
                ...
                do {
                    if let imageData : Data = try Data(contentsOf: imageURL!) {
                        ...
                        if let downloadedImage = image { OperationQueue.main.addOperation() {
